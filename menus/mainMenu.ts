import type { BaseContext, MenuButton } from "../types/index.js";
...
export default class MainMenu extends BaseMenu {
  ...
  private async execCommand(ctx: BaseContext, cmdName: string) {
    ...
    if (!command) {
      return ctx.callbackQuery?.message?.editText(ctx.t("error.command-not-found", { name: cmdName }), { parse_mode: "HTML" });
    }
