				...
				if (CommandClass && CommandClass.prototype instanceof Command) {
					...
					if (command.info.name) {
						...
						if (command.info.aliases && command.info.aliases.length > 0) {
							command.info.aliases.forEach((alias: string) => this.aliases.set(alias.toLowerCase(), command.info.name));
						}
					} else {
						console.warn(`[CommandManager] ⚠️ Skipped ${file}: Command has no name.`);
					}
				} else {
					console.warn(`[CommandManager] ⚠️ Skipped ${file}: Not a Command subclass or no default export.`);
				}
			} catch (error) {
				console.error(`[CommandManager] Ошибка при загрузке команды из файла ${file}:`, error);
			}
		}));
	...
	async loadCommand(commandPath: string) {
		try {
			...
			if (CommandClass && CommandClass.prototype instanceof Command) {
				...
				if (command.info.name) {
					this.commands.set(command.info.name.toLowerCase(), command);
					if (command.info.aliases) command.info.aliases.forEach(alias => this.aliases.set(alias.toLowerCase(), command.info.name));
				}
			}
		} catch (error) {
			console.error(`[CommandManager] Ошибка при загрузке команды из файла ${commandPath}:`, error);
		}
	}
}
