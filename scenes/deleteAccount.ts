...
const deleteAccountScene: WizardScene<CallbackContext> = {
	...
	steps: [
		async (ctx) => {
			...
			if (!accountId) {
				await ctx.callbackQuery?.message?.editText("❌ Ошибка: не удалось определить счёт для удаления.", {
			...
			await ctx.callbackQuery?.message?.editText(`Вы уверены, что хотите удалить счёт ${account?.account_number}?`, {
		...
		async (ctx) => {
			...
			if (ctx.callbackQuery?.data === "confirm") {
				...
				await ctx.callbackQuery.message?.editText("✅ Счёт и все связанные данные успешно удалёны.", {
					reply_markup: new InlineKeyboard()
						.text('Назад', 'utilities-menu')
				});
			}
