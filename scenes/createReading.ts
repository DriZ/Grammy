...
export default class CreateReadingScene extends BaseScene {
  ...
  private handleDateSelection = async (ctx: CallbackContext) => {
    ...
    if (reading.length > 0) {
      await ctx.callbackQuery.message?.editText(ctx.t("create-reading.error-exists", { date: `${month}.${year}` }), {
        reply_markup: new InlineKeyboard()
          .text(ctx.t("button.add-more"), "add_more")
          .row()
          .text(ctx.t("button.back"), `readings-${ctx.wizard.state.accountId}`),
        parse_mode: "HTML"
      });
  ...
  private handleZoneValue = async (ctx: CallbackContext) => {
    if (await this.checkCancel(ctx, ctx.t("create-reading.cancelled"), `readings-${ctx.wizard.state.accountId}`)) return;
    ...
    if (isNaN(value) || value < prevValue) {
      ...
      await ctx.wizard.state.message?.editText(ctx.t("create-reading.error-value-lower", { prev: prevValue }), {
  ...
  private saveReading = async (ctx: CallbackContext) => {
    try {
      ...
      let resultMessage = ctx.t("create-reading.success") + "\n\n";
      const previousReading: IUtilitiesReading | null = ctx.wizard.state.previousReading;
      ...
      if (previousReading) {
        ...
        if (consumptionLines.length > 0) {
          resultMessage += ctx.t("create-reading.consumption") + ":\n" + consumptionLines.join("\n");
        }
      }

      const keyboard = new InlineKeyboard()
        .text(ctx.t("button.add-more"), "add_more").success().row()
        .text(ctx.t("button.back"), `readings-${ctx.wizard.state.accountId}`);
      ...
      console.error(error);
      await ctx.wizard.state.message?.editText(ctx.t("create-reading.error"), { parse_mode: "HTML" });
    }
