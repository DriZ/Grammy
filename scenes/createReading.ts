...
const createReadingScene: WizardScene<CallbackContext> = {
	...
	steps: [
		...
		async (ctx) => {
			...
			await ctx.wizard.params.message?.editText(`📅 Выберите месяц (${currentYear}):`, {
		...
		async (ctx) => {
			...
			if (monthData) {
				...
				if (account.resource === "electricity" && account.meterType) {
					...
					if (account.meterType === MeterType.MULTI_ZONE)
						zones = ["peak", "half-peak", "night"];
				}
				...
				await ctx.wizard.params.message?.editText(
						...
						zoneValues.map((z: ZoneReading) => `${z.name}: ${z.value}`).join("\n"),
					{
						reply_markup: new InlineKeyboard().text(
							"⬅️ Назад",
							`account-${ctx.wizard.params.accountId}`,
						),
					},
				);
			} catch (err) {
				await ctx.wizard.params.message?.editText(
					"⚠️ Ошибка: показания за этот месяц уже внесены.",
					{
						reply_markup: new InlineKeyboard().text(
							"⬅️ Назад",
							`account-${ctx.wizard.params.accountId}`,
